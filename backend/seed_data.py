...
# scriptedspython/demoos/demoos-88c5b0a7b388c582eab72b7e23a82eab7e4cb7c4/backend/seed_data.py
...
def wipe_database():
    print("🗑️  Wiping existing tables using CASCADE...")
    try:
        # Forcefully drops all tables and constraints in PostgreSQL
        with engine.connect() as conn:
            conn.execute(text("DROP SCHEMA public CASCADE;"))
            conn.execute(text("CREATE SCHEMA public;"))
            conn.commit()
    except Exception as e:
        print(f"⚠️ Drop schema failed (might not be Postgres or first run): {e}")
        # Fallback to standard drop if needed
        SQLModel.metadata.drop_all(engine)
...
def seed_data():
    with Session(engine) as session:
        ...
        # --- 5. Create 50 Students (ALL IN SEMESTER 6) ---
        print("🎓 Creating 50 Students with Full Details (All Sem 6)...")
        reg_start = 2301105217
        pass_start = 427001
        ...
        for i in range(50):
            ...
            # Updated with new columns for the HOD search feature
            s_profile = Student(
                user_id=s_user.id, 
                roll_no=f"CSE-{100+i}",      # Using a unique roll number
                regd_no=reg_no,              # The 10-digit number as registration
                name=s_user.full_name,
                department_id="CSE", 
                semester=6,
                email=f"student{i+1}@igitsarang.ac.in",
                contact_no=f"9000000{i:03d}",
                guardian_name=f"Guardian of Student {i+1}",
                guardian_contact_no=f"8000000{i:03d}"
            )
            session.add(s_profile)
        
        session.commit()
        print("✅ Seeding Complete! All 50 students generated with full HOD dashboard info.")
